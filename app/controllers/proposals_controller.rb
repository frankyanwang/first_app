class ProposalsController < ApplicationController
  ...
  def proposals_to_me
    proposals_to_me = Proposal.where(:post_id => current_user.posts)
    respond_to do |format|
      # format.html { render "myproposals"}
  ...
  def accept
    @proposal = Proposal.where(:post_id => current_user.posts, :id => params[:id]).first
    if @proposal
      @proposal.status_type = :accept
      if @proposal.save
        redirect_to new_post_url, :notice  => "Accept the proposal."
        # render :nothing => true, :status => :ok  #200
      else
        # render :nothing => true, :status => :internal_server_error #500
        redirect_to new_post_url, :notice  => "Failed to accept proposal, try again."
      end      
    else
        # render :nothing => true, :status => :not_found #404
        redirect_to new_post_url, :notice  => "Failed to accept proposal, try again later."
    end
  end
  ...
  def reject
    ...
    if @proposal
      ...
      if @proposal.save
        redirect_to new_post_url, :notice  => "Reject the proposal."
        # render :nothing => true, :status => :ok  #200
      else
        redirect_to new_post_url, :notice  => "Failed to reject the proposal, try again."
        # render :nothing => true, :status => :internal_server_error #500
      end      
    else
      redirect_to new_post_url, :notice  => "Failed to reject the proposal, try again later."
        # render :nothing => true, :status => :not_found #404
    end    
  end
  ...
  def update
    ...
    if @proposal.update_attributes(params[:proposal])
      redirect_to new_post_url, :notice  => "Successfully updated proposal."
    else
      render :action => 'edit'
    end
  end
  ...
  def destroy
    @proposal = current_user.proposals.find(params[:id])
    @proposal.destroy
    redirect_to new_post_url, :notice => "Successfully destroyed proposal."
  end
end
